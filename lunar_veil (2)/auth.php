 ...
 * This MUST match the ID mapping in profile.html.
 * * New Role Mapping:
 * 0: Standard Customer
 * 1: Staff
 * 2: Manager
 ...
 * This MUST match the role names used in profile.html.
 */
...
function handleRegister($conn, $input) {
    ...
    $input_role_name = trim($input['role'] ?? 'customer'); // Get the text name (e.g., "VIP CUSTOMER")
    
    // *** FIX: Convert role name to ID ***
    $role_id = getRoleID($input_role_name); // Convert to the integer ID (0, 1, 2, 3, 4, 5)
    ...
    try {
        ...
        // FIX: The 'role' column now binds the integer ID and must be an INT type in the DB.
        $sql = "INSERT INTO users (name, email, age, city_address, birthdate, role, password) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        
        // Data Types: s, s, i, s, s, i, s (Changed 'role' binding from 's' to 'i' for integer)
        $stmt->bind_param("ssissis", 
            ...
            $city_address,      
            $birthdate,         
            $role_id,           // Now binding the integer ID
            $password_hash      
        );
        $stmt->execute();
        ...
        $_SESSION['role'] = $role_id; // Store the ID in the session

        // Get the descriptive name for the response
        ...
        echo json_encode([
            ...
            'username' => $username,
            'role' => $role_name_response // Send the name for confirmation
        ]);
...
function handleLogin($conn, $input) {
    ...
    // Role is expected to be a numeric ID from the database now
    ...
    $_SESSION['role'] = (int)$user['role']; // Store the ID in the session as an integer
...
function handleProfile($conn) {
    ...
    try {
        ...
        if ($result->num_rows === 1) {
            ...
            // CRITICAL FIX: Ensure 'role' is an integer before sending
...
function handleStatus($conn) {
    if (isset($_SESSION['user_id'])) {
        ...
        // FIX: Get the role ID from the session, defaulting to 0
        $role_id = $_SESSION['role'] ?? 0; 
        
        // FIX: Convert the ID to a name for status reporting
        ...
        echo json_encode([
            ...
            'role' => $role_name, // Report the descriptive name
            'balance' => $balance
        ]);
    } else {
        echo json_encode([
